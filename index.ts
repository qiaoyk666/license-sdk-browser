...
export class Client {
    ...
    maxReconnectAttempts: number = 5 // 最大重连次数，-1表示无限重连
    ...
    private async connectWebSocket() {
        ...
        // 启动websocket监听
        this.handleWebSocket(this.ws)


    }

    private handleWebSocket(ws: WebSocket) {
        this.handleBrowserWs(this, ws)
    }
    ...
    private handleBrowserWs(client: Client, ws: WebSocket) {
        ...
        ws.onmessage = function incoming(data: any) {
            ...
            switch(dataObj.msgType){
                case WsMsgType.WsMsgTypePermissionTree:
                    client.module = client.verifyModuleMsg(dataObj)
                    client.emit(EventType.LicenseChange, client.module)
                    break;
                case WsMsgType.WsMsgTypeExpireWarning:

                    // 数据（必须是Uint8Array或者Buffer格式）
                    // const message  = Buffer.from(dataObj.msg, 'base64');
                    const message = client.base64ToUint8Array(dataObj.msg)
                    const decoder = new TextDecoder();
                    const decodedString = decoder.decode(message);
                    const messageObj = JSON.parse(decodedString)
                    client.emit(EventType.LicenseExpiring, messageObj)
                    break;

                case WsMsgType.WsMsgTypeRevokeLicense:
                    const msgArray = client.base64ToUint8Array(dataObj.msg)
                    const d = new TextDecoder();
                    const decodedStr = d.decode(msgArray);
                    const msgObj = JSON.parse(decodedStr)
                    client.emit(EventType.LicenseRevoke, msgObj)
                    break;

                case WsMsgType.MsgTypeHeartbeat:
                    const msgArrayHB = client.base64ToUint8Array(dataObj.msg)
        ...
        ws.onclose = async function close() {
            ...
            await client.delay(client.reconnectWaitTimeSecond)
            await client.reconnect()
        }

       
        ws.onerror = function errorFun(err: any) {
            console.error(`Websocket Error: ${err}`)
            console.log(err)
            client.emit(EventType.ConnectionError, err)
            throw new Error("ws连接异常")
        }
    }    
